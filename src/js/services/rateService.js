...
RateService.prototype.updateRates = function() {
  ...
  var rateServiceUrl = 'https://masq.bzedge.org/rates';

  function getBZC(cb, tries) {
    tries = tries || 0;
    if (!self.httprequest) return;
    if (tries > 5) return cb('could not get BZC rates');
...
RateService.prototype.toFiat = function(satoshis, code, chain) {
  ...
  return satoshis * this.SAT_TO_BZC * this.getRate(code, chain);
};
...
RateService.prototype.fromFiat = function(amount, code, chain) {
  ...
  return amount / this.getRate(code, chain) * this.BZC_TO_SAT;
};
